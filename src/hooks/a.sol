...
contract Jackpot is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    address public superJackpotWallet;
    ...
    uint256 public superJackpotPercent = 10;
    ...
    uint256 public DRAWING_INTERVAL = (6 hours); 
    uint256 public nextDrawing = 0;
    uint private constant MAX_DRAW_PARTICIPANTS = 1000; 
    uint private _currentBuyIndex = 0;
    bool private _maxDrawOverun = false;
    address[MAX_DRAW_PARTICIPANTS] _lastBuyersList;
    mapping (address => uint ) private _lastBuyersMap; // keeps index of address in _lastBuyersList + 1 !!
    ...
    uint256 public numTokensSellToAddToLiquidity = 69000000 * 10**9;  //TODO Prod 10000000000 * 10**9
    ...
    event DrawingSlot(address participant, uint256 amount);
    event ForfeitSlot(address participant);
    event SlotExpired(address participant);
    event SuperJackpotShare(address pool, uint256 amount);
    event Draw(address winner, uint256 amount);
    event CleanSlotsList();
    ...
    constructor () {
        ...
        //console.log("Pair: ", uniswapV2Pair);
        ...
        // set initial super jackpot pool to contract itself
        superJackpotWallet = address(this);
    ...
    function setSuperJackpotWallet(address walletAddress) public onlyOwner {
        superJackpotWallet = walletAddress;
    }
    ...
    function setDrawingInterval(uint256 _hours) external onlyOwner {
        require(_hours < 7*24, "Interval too big.");
        DRAWING_INTERVAL = _hours * (1 hours);
    }
    ...
    function checkSwap(address _from, address _to, uint256 amount) internal {
        
        if (_to != uniswapV2Pair && _to != address(uniswapV2Router) && _from != uniswapV2Pair && _from != address(uniswapV2Router)) {
            // Regular transfer
            
            if (_lastBuyersMap[_from] != 0) {
               // Remove from the list and add contract address to increase next jackpot
               _lastBuyersList[_lastBuyersMap[_from]-1] = address(this);
               _lastBuyersMap[_from] = 0;
               emit ForfeitSlot(_from);
            }
        }
        
        
        if (_to == uniswapV2Pair && _from != address(uniswapV2Router)) {
            // Sale
        ...
        if (_from == uniswapV2Pair && _to != address(uniswapV2Router)) {
            ...
            if (_lastBuyersMap[_to] == 0) {
                if (_lastBuyersList[_currentBuyIndex] != 0x0000000000000000000000000000000000000000) {
                   ...
                   _lastBuyersMap[_lastBuyersList[_currentBuyIndex]] = 0;
                   emit SlotExpired(_lastBuyersList[_currentBuyIndex]);
                 }
               _lastBuyersList[_currentBuyIndex] = _to;
               _currentBuyIndex++;
               _lastBuyersMap[_to] = _currentBuyIndex;
            }
    ...
    function draw() public nonReentrant {
        ...
        // Reset entries map
        for (uint256 index = 0; index < MAX_DRAW_PARTICIPANTS; index++) {
            _lastBuyersMap[_lastBuyersList[index]] = 0;
            _lastBuyersList[index] = 0x0000000000000000000000000000000000000000;
            _currentBuyIndex = 0;
            _maxDrawOverun = false;
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // super jackpot share
        uint256 supershare = newBalance.mul(superJackpotPercent).div(100);
        payable(superJackpotWallet).transfer(supershare);
        emit SuperJackpotShare(superJackpotWallet, supershare);
        
        newBalance -= marketingshare;
        newBalance -= winnershare;
        newBalance -= supershare;
